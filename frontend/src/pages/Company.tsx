...
export default function Company() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold text-white">Kualitas Produk & Finansial</h3>
        <p className="text-sm text-slate-400">Track mutu CPO dan performa finansial perusahaan dalam satu tampilan terpadu.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <KPICard title="Pendapatan" value={`Rp ${totals.revenue.toLocaleString()}`} hint="akumulasi YTD" />
        <KPICard title="COGM" value={`Rp ${totals.cogm.toLocaleString()}`} hint="biaya produksi" />
        <KPICard title="OPEX" value={`Rp ${totals.opex.toLocaleString()}`} hint="operasional" />
        <KPICard title="Laba Kotor" value={`Rp ${totals.gp.toLocaleString()}`} hint="setelah biaya" />
      </div>
      {qualityAlert && (
        <div className="rounded-2xl border border-amber-400/50 bg-amber-500/15 px-6 py-4 text-amber-100">
          ...
          <div className="text-sm text-amber-100/80">Cek FFA/Moisture di tanggal {qualityAlert.date} (Site: {qualityAlert.site})</div>
        </div>
      )}
