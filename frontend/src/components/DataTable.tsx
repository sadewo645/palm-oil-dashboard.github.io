    ...
    <div className="card card-pad overflow-auto">
      <table className="min-w-full text-sm text-slate-200">
        <thead className="text-xs uppercase tracking-wide text-slate-400">
          <tr>
            {columns.map((c) => (
              <th key={c} className="text-left font-medium pb-3 pr-6">{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
      ...
      {!rows.length && (
        <div className="py-6 text-center text-sm text-slate-400">Belum ada data tersedia untuk tabel ini.</div>
      )}
    </div>
  );
}
